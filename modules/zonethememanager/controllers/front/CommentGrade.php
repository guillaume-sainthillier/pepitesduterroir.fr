...
class ZOneThemeManagerCommentGradeModuleFrontController extends ModuleFrontController
{
    public function displayAjax()
    {
        ...
        if ($idProducts) {
            ...
            $array_id_product = Tools::jsonDecode($idProducts);
            foreach ($array_id_product as $id_product) {
                $productCommentsNb = $productCommentRepository->getCommentsNumber($id_product, Configuration::get('PRODUCT_COMMENTS_MODERATE'));
                if ($productCommentsNb) {
                    $averageGrade = $productCommentRepository->getAverageGrade($id_product, Configuration::get('PRODUCT_COMMENTS_MODERATE'));
                    
                    $results[$id_product] = array(
        ...
        $this->ajaxRender(Tools::jsonEncode($results));
    }
}
